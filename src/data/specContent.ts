...
export const specSections: SpecSection[] = [
  {
    ...
    title: 'Git 协作与分支规范',
    category: '协同管理',
    summary: '采用单主干 + 特性分支轻量级 Flow，严禁直接向 main 推代码，规范 Commit 格式。',
    keyPoints: [
      '分支策略：main(生产演示) + dev(集成分支) + feature/xxx(个人特性) + fix/xxx(缺陷修复)',
      '提交格式必须遵循 Conventional Commits：feat/fix/docs/style/refactor/test',
      '安全红线：严禁将任何 LLM API Key、密码上传至 GitHub，必须配置 .gitignore 与脱敏模版',
    ],
  },
  {
    id: 'backend',
    title: 'Java / Spring Boot 编码规范',
    category: '后端架构',
    summary: '标准 MVC 4 层架构职责解耦，使用 MyBatis-Plus，统一 Result<T> 响应与全局异常拦截。',
    keyPoints: [
      '严格分层：controller(无业务逻辑) -> service -> rag(AI增强) -> dao -> model(entity/dto/vo)',
      '统一使用 Result<T> 封装，全局 @RestControllerAdvice 捕获异常，严禁把堆栈抛给前端',
      '持久化安全：禁止 SELECT *，统一配置逻辑删除与 created_at / updated_at 审计字段',
      '入参校验：Controller 层全面推行 @Valid + JSR-303 注解验证，防非法空参输入',
    ],
  },
  {
    id: 'frontend',
    ...
    category: '前端工程',
    summary: '全面采用 <script setup lang="ts"> 组合式 API，严格显式定义 Props/Emits 类型。',
    keyPoints: [
      '规范写法：单文件组件统一采用 <script setup lang="ts">，禁止混用 Vue2 Options API',
      '状态与路由：Pinia 模块化管理用户信息与当前会话，路由守卫拦截未登录访问',
      '流式中断机制：使用 fetchEventSource 时必须在组件销毁或切换会话时调用 abort() 防止内存泄漏',
      'XSS 安全渲染：Markdown 输出必须经过 DOMPurify 转义过滤，保障演示安全性',
    ],
  },
  {
    id: 'api',
    title: 'RESTful API & SSE 流式协议',
    category: '接口协议',
    summary: '资源名词复数与 HTTP 语义化，规定智能答疑 4 种 Event 事件流结构。',
    keyPoints: [
      'URL 规范：小写短横线名词复数，如 /api/teacher/course-documents，禁止在 URL 中动宾混杂',
      '核心 SSE 事件流：event: references(出处首包) -> event: message(打字机吐字) -> event: done / error',
      '接口先行原则：后端必须在 Knife4j 明确字段和返回结构，前端方可对接，杜绝联调猜字段',
    ],
  },
  {
    id: 'rag',
    title: 'AI 与 RAG 专项工程规范',
    category: 'RAG核心',
    summary: '规范分块递归策略、元数据必填项、Top-K 检索阈值与防幻觉 System Prompt。',
    keyPoints: [
      '分块标准：Chunk Size = 400 字符，Overlap = 50 字符，递归字符切分优先以段落和标点断开',
      '元数据注入：每个切块必须附带 courseId(课程隔离)、docId、fileName、chunkIndex',
      '防幻觉调优：智能答疑设置 temperature = 0.2，设置 Cosine 相似度 >= 0.70 最低过滤阈值',
      '严格提示词：指令要求未命中时回答“当前课件未明确说明”，禁止模型随意脑补',
    ],
  },
  {
    id: 'env',
    title: '开发环境与日常联调纪律',
    category: '团队纪律',
    summary: '统一 JDK 17、Node 18/20、MySQL 8.0 基线，实施先拉后提与每周冒烟测试。',
    keyPoints: [
      '开发基线：JDK 17 LTS + Maven 3.8+ + Node.js 18+ + MySQL 8.0(utf8mb4) + Chroma Docker',
      '联调守则：每天开工前 git pull origin dev；提交 PR 前必须在本地编译通过并通过冒烟测试',
      '例行同步：每周一晚 15 分钟线上进度对齐，消除技术阻塞点，保障项目准时交付',
    ],
  },
];

export const devSpecificationMarkdown = `# AI 驱动的在线学习智能答疑辅导平台（RAG 课程知识库）
...
### 2.4 数据库操作与 SQL 规范
- **ID 生成策略**：统一使用雪花算法（Snowflake / \`IdType.ASSIGN_ID\`）或 MySQL 自增主键。
- **软删除与审计字段**：重要业务表必须包含：
...
- **样式规范**：统一使用 \`<style scoped>\` 或 Tailwind CSS 工具类，严禁污染全局样式选择器。主色采用 Indigo \`#4F46E5\`。

### 3.3 SSE 流式通信与 Markdown 渲染安全
- **流式请求管理**：在用户切换会话或离开页面时，**必须调用 \`abort()\` 中断未完成的 SSE 流**，防止内存泄漏和后端无意义调用。
...
- HTTP 动词语义化：\`GET\`（查询）、\`POST\`（创建/复杂查询）、\`PUT/PATCH\`（更新）、\`DELETE\`（删除）。
...
1. **\`event: references\`**（首包下发，检索到的知识库出处 JSON 数组）
2. **\`event: message\`**（增量生成，流式打字机吐字片段）
3. **\`event: done\`**（结束标记包，携带生成的 recordId 与状态）
4. **\`event: error\`**（异常中断包，携带 errorCode 与友好说明）
...
- **断句优先**：必须采用递归字符切分器（RecursiveCharacterSplitter），优先以换行与句号作为断点，**严禁从单词或公式中间切断**。
- **元数据必填项**：切出的每一个分块必须注入 \`courseId\`、\`docId\`、\`fileName\`、\`chunkIndex\`。

### 5.2 向量检索与防幻觉 Prompt 规范
- **检索阈值**：Top-K 取值推荐 \`K = 3 ~ 4\`，设置相似度最低阈值（Cosine 相似度 >= 0.70），低于阈值的片段抛弃。
...
### 6.2 联调与代码合并纪律
1. **先拉取后提交**：每天开始写代码前，先 \`git pull origin dev\`；提交 PR 前，先在本地编译通过。
2. **前后端接口先行**：成员 B 必须先输出 Knife4j 接口定义，成员 C/D 方可开始调用。
3. **冒烟测试再合并**：任何 PR 合并至 \`dev\` 分支前，必须完成基础闭环冒烟测试。
4. **每周定期同步**：每周一晚组织 15 分钟线上站会，对齐里程碑产出与技术阻塞点。
`;
